...
voice for the Speect Speech Synthesis System, given a number of basic
resources. Due to the flexibility of Speect, this is not a
...
A ``voice`` in Speect consists of a collection of ``data resources``,
code modules or ``plugins`` and a single ``definition`` file
describing how these are used during synthesis.
...
``./examples/voice.*.json``. For more information on the ``voice
definition`` file refer to the Speect documentation.
